...
async function fetchProofFiles(db, reportId) {
    const [proofs] = await db.query(
        `SELECT file_url FROM report_proofs WHERE report_id = ?`,
        [reportId]
    );
    return proofs;
}
...
async function deleteProofRecords(db, reportId) {
    await db.query(`DELETE FROM report_proofs WHERE report_id = ?`, [reportId]);
}

/** Delete the report record itself */
async function deleteReportRecord(db, reportId) {
