...
const ChatWindow = ({ selectedUser }) => {
  ...
  const { data: profileData } = useUserProfile(ROLE.BUSINESS_EMPLOYER);
  const currentUserId = profileData?.user_id;

  const { conversation_id = null } = selectedUser || {};
  const { data: messages = [], isLoading, isError } = useMessageHistory(ROLE.BUSINESS_EMPLOYER, conversation_id);
