...
const Message = () => {
  ...
  const mutation = useSendMessage('jobseeker', user_id);
  const { messages } = useChat(selectedConversationId, user_id, 'jobseeker');
  ...
  const handleSend = () => {
    ...
    // ✅ Send full `conversation_id` to the hook
    mutation.mutate({
      receiver_id,
      message_text: text,
      file,
  ...
  useEffect(() => {
    const loadConversations = async () => {
      try {
        const conversations = await messageApi.fetchConversations('jobseeker');
  ...
  // For messages fetching and infinite scroll
  useEffect(() => {
