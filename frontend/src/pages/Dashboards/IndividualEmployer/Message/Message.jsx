...
import { useIndividualEmployerProfile, useAllUsers } from '../../../../../hooks/useUserProfiles';
...
const Message = () => {
  ...
  const { data: profile } = useIndividualEmployerProfile();
  const user_id = profile?.user_id;
  const mutation = useSendMessage('individual-employer', user_id);
  const { messages } = useChat(selectedConversationId, user_id, 'individual-employer');
  ...
  useEffect(() => {
    const loadConversations = async () => {
      try {
        const conversations = await messageApi.fetchConversations('individual-employer');
        const messages = await messageApi.fetchMessagesForConversations('individual-employer', conversations);
  ...
  useEffect(() => {
    ...
    const handleIncomingMessage = (newMessage) => {
      // Do not duplicate if it's from an already opened conversation
      ...
      // Optionally highlight it visually
          ...
          <div className="w-2/3 flex flex-col justify-between p-6 relative">
            {selectedConversationId ? (
              <>
                ...
                <MessageInput text={text} setText={setText} file={file} setFile={setFile} onSend={handleSend} />
              </>
            ) : (
