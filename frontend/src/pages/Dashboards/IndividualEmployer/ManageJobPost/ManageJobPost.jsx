...
import Form from '../../IndividualEmployer/Verification Form/Form';
...
const ManageJobPost = () => {
    ...
    } = useStatusChange(ROLES.INDIVIDUAL_EMPLOYER);

    const {
        deleteJobPost,
        isLoading: isDeleting,
        error: deleteError,
    } = useDeleteJobPost(ROLES.INDIVIDUAL_EMPLOYER);
    ...
    const individualEmployerJobs = jobPosts.filter((job) => job.user_id === employer.user_id);


    const pendingJobs = individualEmployerJobs.filter(
        (job) =>
            job.status === 'pending' &&
            job.jobpost_status !== 'deleted' 
    );

    const activeJobs = individualEmployerJobs.filter(
        (job) =>
            job.status === 'approved' &&
    ...
    const completedJobs = individualEmployerJobs.filter(
        (job) =>
            job.status === 'approved' &&
