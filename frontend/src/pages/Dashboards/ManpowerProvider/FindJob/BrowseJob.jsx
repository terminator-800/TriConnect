...
import { useAllUsers, useManpowerProviderProfile } from '../../../../../hooks/useUserProfiles';
...
const BrowseJob = () => {
    ...
    const { data: profileData } = useManpowerProviderProfile()

    const { data: jobPosts = [], isLoading: loadingJobPosts, isError: errorJobPosts } = useJobPosts();
    const { data: users = [] } = useAllUsers();
    const { data: applications = [], isLoading } = useJobApplications(ROLES.MANPOWER_PROVIDER, profileData.user_id);
    ...
    const approvedJobPosts = jobPosts.filter(
        (post) =>
            ...
            post.user_id !== profileData.user_id && 
