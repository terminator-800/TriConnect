const MessageHeader = ({ selectedConversationId, messages, user_id, allUsers }) => {
  ...
  // Manpower provider chats with jobseekers or employers (business/individual)
  const name =
    user?.business_name ||
    (user?.role === 'individual_employer' ? user?.full_name : null) ||
    (user?.role === 'jobseeker' ? user?.full_name : null) ||
    'Unknown';
