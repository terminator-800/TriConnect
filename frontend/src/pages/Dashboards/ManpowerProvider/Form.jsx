...
const ManpowerProviderForm = ({ onClose, onSubmitSuccess }) => {
  ...
  return (
    <div className='fixed flex items-center justify-center z-50 inset-0'>
      ...
      <form onSubmit={submitRequirements} className='relative z-10 border-2 border-gray-300 bg-white rounded-xl p-6 h-[90vh] overflow-y-auto w-full max-w-2xl mt-20'>
        ...
        <div className='mt-6'>
          ...
          <FileUpload
            ref={DOLERef}
            label="DOLE (Department of Labor and Employment) Certificate"
            description="Upload a clear image of your DOLE membership"
            file={DOLE}
            setFile={setDOLE}
            preview={DOLEPreview}
            setPreview={(v) => v === 'modal' ? setShowDoleModal(true) : setDOLEPreview(v)}
            id="DOLE"
            allowPreview
          />
          ...
          <FileUpload
            ref={mayorBirRef}
            label="Mayor’s Permit or BIR Registration"
            description="Upload this documents for enhanced verification"
            file={mayor_permit_or_BIR}
            setFile={setMayor_permit_or_BIR}
            preview={mayor_birPrev}
            setPreview={(v) => v === 'modal' ? setShowMayor_BirModal(true) : setMayor_birPrev(v)}
            id="mayor_permit_or_BIR"
            allowPreview
          />

          <FileUpload
            ref={agencyProofRef}
            label="Other certifications based on agency type"
            description="Upload recent certificates for enhanced verification"
            file={agency_proof}
            setFile={setAgency_proof}
            preview={agency_proofPreview}
            setPreview={(v) => v === 'modal' ? setShowAgency_ProofModal(true) : setAgency_proofPreview(v)}
            id="agency_proof"
            allowPreview
          />
          ...
          <FileUpload
            ...
            file={authorized_agency_id}
            setFile={setAuthorized_agency_id}
            preview={authorized_agencyPreview}
            setPreview={(v) => v === 'modal' ? setShowAuthorized_agencyPreview(true) : setAuthorized_agencyPreview(v)}
            id="authorized_agency_id"
            allowPreview
          />
        </div>
